...
def _grok_apply_signals(session_dir, stats):
    ...
    # Also extract cache_hit_rate from signals.json if present
    if "lastCacheHitRate" in sig:
        stats["cache_hit_rate"] = sig["lastCacheHitRate"]
        applied = True
...
def _grok_iter_usage_snapshots(session_dir):
    ...
    Grok Build format: params.update.usage with sessionUpdate="turn_completed"
    """
        ...
        with open(updates_file, encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                if not isinstance(update, dict):
                    continue
                # Only process turn_completed events with usage data
                if update.get("sessionUpdate") != "turn_completed":
                    continue
                usage = update.get("usage")
                if not isinstance(usage, dict):
                    continue
                snap = _grok_parse_usage_object(usage)
                if snap is not None:
                    yield snap
    except OSError:
        return
