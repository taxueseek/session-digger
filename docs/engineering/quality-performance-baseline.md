...
**修法（v0.9.23.5）：从索引自身把环境推导回来**（`echolib.adopted_envs` + `_builder._scan_envs`）。报告层把未知 dot-dir 命名为 `dotdir.name.lstrip(".")`，所以环境 id `pi` 的根就是 `~/.pi`——**不需要持久化任何东西**，只要「索引里有该 id 的行」且「该目录还在」，这个环境就仍然活跃，必须留在扫描集合里。

为什么不用另外两条路：
- **自动索引任意未知 dot-dir**：会把 `~/.foo/*.jsonl` 的内容吃进 FTS 与证据投影。报告层只是**列出**目录名，索引内容是完全不同量级的动作，对一个发布给别人用的工具是隐私升级。
- **显式登记进 `KNOWN_UNADAPTED`**：发布版里会出现本机路径。

「只从索引已有的 id 推导」保证发现面**永远不会超过上一次构建已经认定为会话的范围**，且双向收敛（目录删了就不再收养）。

**扫描集合与剪枝集合改为同一函数**（`_scan_envs`）：两个集合必须一致，否则要么留下永不更新的行，要么删掉从没列过的行。

**实测**：收养 6 个环境、183 行全部刷新（`user_evidence_json` 空值 183 → **0**，会话 6535 → 6539，多出 4 个新增会话）；首轮构建 2.97 s（一次性重解析 190 个），随后回到 0.43–0.47 s 且幂等（`indexed: 0`）；发现阶段代价 **+7.1 ms**（`scan_sessions` 171 → 186–197 ms，最贵的 `jimeng-cli` 3.6 ms 是「整个 dot-dir 换 1 个会话」）。路径来自数据，故 id 中的 `/`、`\`、`..`、前导 `.` 一律拒绝。

**残留（刻意保留）**：环境目录被删除后，其行既无法刷新也**不会被删**——保留的是这些会话仍可被 FTS 检索到的历史，而 `_prune_stale_sessions` 的护栏刻意不把「看不见的目录」当成删除证据。代价是这些行的统计停留在最后一天。
