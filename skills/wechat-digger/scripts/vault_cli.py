...
# Shared schema lives one directory up (scripts/wechat_schema.py). This
# script is executed directly as a subprocess and is also shipped
# standalone, so bootstrap the parent directory onto sys.path instead of
# assuming the caller set PYTHONPATH.
...
def username_for_table(table: str, con: sqlite3.Connection) -> str:
    """Reverse-resolve a shard table to its username using the Name2Id map."""
    if not table_username(table):
        return ""
    if not table_exists(con, "Name2Id"):
        return ""
    try:
...
def username_for_table_from_contacts(table: str, contacts: dict[str, dict]) -> str:
    """Reverse-resolve a shard table using the contacts map.

    Both reverse lookups delegate to ``message_table`` so the forward and
    reverse derivations cannot drift apart.
    """
    if not table_username(table):
        return ""
...
def message_columns(con: sqlite3.Connection, table: str) -> dict[str, str]:
    """Resolve logical column roles against this shard's real columns.

    Delegates to the shared schema so the alias table has one definition.
    """
    return resolve_columns(table_columns(con, table))
...
def command_search(args: argparse.Namespace) -> None:
    ...
    if args.chat:
        ...
        for db_path in message_dbs(decrypted_dir):
            with connect(db_path) as con:
                tables = [row["name"] for row in con.execute("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE ?", (message_table_like_pattern(),))]
                name2id = load_name2id(con)
                for table in tables:
...
def _safe_moment(row, contacts):
    """SnsTimeLine 行 → 朋友圈 dict；XML 解析失败不炸，显式标记。"""
    try:
        return parse_moment(row["content"], str(row["tid"]), str(row["user_name"]))
    except Exception as exc:
        return {"tid": str(row["tid"]), "username": str(row["user_name"]),
                "time": "", "timestamp": 0,
                "content": f"[无法解析 XML: {exc}]", "media": [], "links": []}


def _iter_timeline_posts(sns_db, contacts, usernames=None, keyword=None, start_ts=None, end_ts=None, limit=None):
    """SnsTimeLine → 过滤后倒序朋友圈列表（moments/sns-feed/sns-search 共享收集）。

    usernames=None 表示不过滤作者（全时间线）；keyword 对全文（含媒体/链接）匹配。
    """
    posts = []
    with connect(sns_db) as con:
        if usernames:
            placeholders = ",".join("?" for _ in usernames)
            cursor = con.execute(f"SELECT tid, user_name, content FROM SnsTimeLine WHERE user_name IN ({placeholders})", tuple(usernames))
        else:
            cursor = con.execute("SELECT tid, user_name, content FROM SnsTimeLine")
        for row in cursor:
            if not row["content"]:
                continue
            post = _safe_moment(row, contacts)
...
def command_sns_feed(args: argparse.Namespace) -> None:
    """朋友圈时间线全览（wx sns-feed 的本地等价，0.0.9.2 起原生）。"""
    decrypted_dir = resolve_decrypted_dir(args.decrypted_dir)
    sns_db = decrypted_dir / "sns/sns.db"
    if not sns_db.exists():
        raise SystemExit(f"找不到 sns.db: {sns_db}")
    contacts, _ = load_contacts(decrypted_dir)
    posts = _iter_timeline_posts(
        sns_db, contacts, usernames=None,
        keyword=args.keyword,
        start_ts=parse_time(args.start), end_ts=parse_time(args.end, end_of_day=True),
        limit=args.limit,
    )
...
def command_sns_search(args: argparse.Namespace) -> None:
    """朋友圈全文搜索（wx sns-search 的本地等价）：关键词必填、不过滤作者。"""
    if not args.keyword:
        raise SystemExit("请传 --keyword")
    decrypted_dir = resolve_decrypted_dir(args.decrypted_dir)
    sns_db = decrypted_dir / "sns/sns.db"
    if not sns_db.exists():
        raise SystemExit(f"找不到 sns.db: {sns_db}")
...
def command_sns_notifications(args: argparse.Namespace) -> None:
    """朋友圈互动通知（wx sns-notifications 的本地等价）：SnsMessage_tmp3 评论/赞。"""
    decrypted_dir = resolve_decrypted_dir(args.decrypted_dir)
    sns_db = decrypted_dir / "sns/sns.db"
    if not sns_db.exists():
        raise SystemExit(f"找不到 sns.db: {sns_db}")
    ...
    with connect(sns_db) as con:
        for row in con.execute(
            "SELECT local_id, create_time, type, from_username, from_nickname, to_nickname, content "
            "FROM SnsMessage_tmp3 ORDER BY create_time DESC LIMIT ?",
            (args.limit,),
        ):
            items.append({
                "type": {1: "like", 2: "comment"}.get(row["type"], str(row["type"])),
                "time": fmt(row["create_time"]),
                "from": row["from_nickname"] or row["from_username"],
                "to": row["to_nickname"] or row["to_username"],
                "content": row["content"] or "",
            })
    output({"count": len(items), "notifications": items}, args.format)


_ZSTD_MAGIC = b"\x28\xb5\x2f\xfd"


def _wcdb_blob_text(content, comp):
    """biz 库正文取数：message_content 实测存 WCDB 内建 zstd 压缩的 BLOB
    （标准 zstd magic 28b52ffd，wcdb_builtin_compression_record 表登记压缩列），
    直接 zstd 解压还原 XML；明文 str 原样返回；压缩分支失败显式标记不炸。"""
    if isinstance(content, (bytes, memoryview)):
        b = bytes(content)
        if b[:4] == _ZSTD_MAGIC:
            try:
                import zstandard as _zstd
                b = _zstd.ZstdDecompressor().decompress(b)
            except Exception:
                return "[压缩内容解析失败]"
    ...
    if comp:
        try:
            import zstandard as _zstd
            return _zstd.ZstdDecompressor().decompress(bytes(comp)).decode("utf-8", "ignore")
        except Exception:
            return "[压缩内容解析失败]"
    return content or ""


def command_biz_articles(args: argparse.Namespace) -> None:
    """公众号文章/推送（wx biz-articles 的本地等价）：biz_message_0 全表聚合。

    每表一次小查询 + Python 端归并，XML 解析 title/公众号名/链接；
    表名经 biz 库自带 Name2Id 反查公众号 username。
    """
    decrypted_dir = resolve_decrypted_dir(args.decrypted_dir)
    biz_db = decrypted_dir / "message/biz_message_0.db"
    if not biz_db.exists():
        raise SystemExit(f"找不到 biz_message_0.db: {biz_db}")
    start_ts = parse_time(args.start)
    end_ts = parse_time(args.end, end_of_day=True)
    import re as _re
    from xml.etree import ElementTree as _ET
    items = []
    with connect(biz_db) as con:
        # biz 的 Name2Id 是 (user_name, is_session)，rowid 即序号——与 real_sender_id 同域
        n2i = {rowid: uname for rowid, uname in con.execute("SELECT rowid, user_name FROM Name2Id")} if con.execute(
            "SELECT COUNT(*) FROM sqlite_master WHERE name='Name2Id'").fetchone()[0] else {}
        ...
        for t in tabs:
            try:
                cur = con.execute(
                    f"SELECT server_id, real_sender_id, create_time, message_content, compress_content "
                    f"FROM {t} WHERE message_content IS NOT NULL AND message_content != '' "
                    f"ORDER BY create_time DESC LIMIT ?", (args.limit,))
            except Exception:
                continue
            for server_id, sender_id, ts, content, comp in cur:
                content = _wcdb_blob_text(content, comp)
                if start_ts and (not ts or ts < start_ts):
                    continue
                if end_ts and (not ts or ts > end_ts):
                    continue
                title = desc = url = ""
                m = _re.search(r"<title>(.*?)</title>", content or "", _re.S)
                title = m.group(1)[:120] if m else ""
                title = _re.sub(r"^<!\[CDATA\[|\]\]>$", "", title).strip()
                m = _re.search(r"<des>(.*?)</des>", content or "", _re.S)
                desc = m.group(1)[:200] if m else ""
                m = _re.search(r"<url>(.*?)</url>", content or "", _re.S)
                url = m.group(1)[:500] if m else ""
                nickname = ""
                m = _re.search(r'nickname="([^"]*)"', content or "")
                nickname = m.group(1) if m else (n2i.get(int(sender_id), "") if sender_id else "")
                items.append({
                    "time": datetime.fromtimestamp(ts).strftime("%Y-%m-%d %H:%M") if ts else "-",
                    "biz": nickname or f"u:{sender_id}",
                    "title": title,
                    "desc": desc,
                    "url": url,
                    "table": t,
    ...
    output({"count": len(items), "articles": items}, args.format)
...
def command_moments(args: argparse.Namespace) -> None:
    ...
    end_ts = parse_time(args.end, end_of_day=True)
    posts = _iter_timeline_posts(
        sns_db, contacts, usernames=usernames or None,
        keyword=args.keyword, start_ts=start_ts, end_ts=end_ts, limit=args.limit,
    )
    data = {"count": len(posts), "moments": posts}
    output(data if args.format == "json" else render_moments_text(posts), args.format)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("sns-feed", help="朋友圈时间线全览（不过滤作者）")
    ...
    p.set_defaults(func=command_sns_feed)

    p = sub.add_parser("sns-search", help="朋友圈全文搜索")
    ...
    p.add_argument("--decrypted-dir", default=None)
    p.add_argument("--format", default="json", choices=["json", "text"])
    p.set_defaults(func=command_sns_search)

    p = sub.add_parser("sns-notifications", help="朋友圈互动通知（评论/赞）")
    p.add_argument("--limit", type=int, default=50)
    p.add_argument("--decrypted-dir", default=None)
    p.add_argument("--format", default="json", choices=["json", "text"])
    p.set_defaults(func=command_sns_notifications)

    p = sub.add_parser("biz-articles", help="公众号文章/推送")
    p.add_argument("--start", default=None)
    p.add_argument("--end", default=None)
    p.add_argument("--limit", type=int, default=50)
    p.add_argument("--decrypted-dir", default=None)
    p.add_argument("--format", default="json", choices=["json", "text"])
    p.set_defaults(func=command_biz_articles)
